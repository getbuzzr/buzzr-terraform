...
resource "aws_elastic_beanstalk_application" "buzzr_staging" {
  name        = "buzzr-staging-app"
...
resource "aws_elastic_beanstalk_application_version" "buzzr_staging_application" {
...
resource "aws_elastic_beanstalk_environment" "buzzr_staging_env" {
  ...
  setting {
    ...
    value     = aws_acm_certificate.staging_api_getbuzzr_co.arn
  }
  setting {
    namespace = "aws:elbv2:listener:443"
    name      = "SSLPolicy"
