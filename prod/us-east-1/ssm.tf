...
resource "aws_ssm_parameter" "api_db_database_uri" {
  ...
  type  = "SecureString"
  value = "mysql://${aws_rds_cluster.prod_buzzr_aurora_cluster.master_username}:${data.aws_ssm_parameter.api_db_server_password.value}@${aws_rds_cluster_endpoint.buzzr_prod_endpoint.endpoint}/${aws_rds_cluster.prod_buzzr_aurora_cluster.database_name}"
}
...
resource "aws_ssm_parameter" "api_ecr_repo" {
  name  = "api_ecr_repo"
  type  = "SecureString"
  value = aws_ecr_repository.buzzr_prod_api.repository_url
}
